...
def availability_set_create_or_update(
    ...
    compconn = __utils__["azurerm.get_client"]("compute", **kwargs)
    ...
    try:
        setmodel = __utils__["azurerm.create_object_model"]("compute", "AvailabilitySet", **kwargs)
    ...
    try:
        ...
        __utils__["azurerm.log_cloud_error"]("compute", str(exc), **kwargs)
...
def availability_sets_list(resource_group, **kwargs):
    ...
    compconn = __utils__["azurerm.get_client"]("compute", **kwargs)
    try:
        avail_sets = __utils__["azurerm.paged_object_to_list"](
...
def availability_sets_list_available_sizes(
    ...
    compconn = __utils__["azurerm.get_client"]("compute", **kwargs)
    try:
        sizes = __utils__["azurerm.paged_object_to_list"](
...
def virtual_machines_list(resource_group, **kwargs):
    ...
    compconn = __utils__["azurerm.get_client"]("compute", **kwargs)
    try:
        vms = __utils__["azurerm.paged_object_to_list"](
...
def virtual_machines_list_all(**kwargs):
    ...
    compconn = __utils__["azurerm.get_client"]("compute", **kwargs)
    try:
        vms = __utils__["azurerm.paged_object_to_list"](compconn.virtual_machines.list_all())
        for vm in vms:  # pylint: disable=invalid-name
            result[vm["name"]] = vm
    except CloudError as exc:
        __utils__["azurerm.log_cloud_error"]("compute", str(exc), **kwargs)
