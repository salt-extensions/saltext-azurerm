"""
Azure Resource Manager Resource State Module
...
def __virtual__():
    """
    Only make this state available if the azurerm_resource module is available.
    """
    if "azurerm_resource.resource_group_check_existence" in __salt__:
        return __virtualname__
    return (False, "azurerm_resource module could not be loaded")
...
def resource_group_absent(name, connection_auth=None):
    ...
    group = __salt__["azurerm_resource.resource_group_get"](name, **connection_auth)
    deleted = __salt__["azurerm_resource.resource_group_delete"](name, **connection_auth)
    ...
    **kwargs,
    ...
    .. code-block:: yaml
        ...
        Ensure policy definition exists:
            azurerm_resource.policy_definition_present:
                - name: testpolicy
    ...
    policy = __salt__["azurerm_resource.policy_definition_create_or_update"](
        name=policy_name,
        policy_rule=policy_rule,
        policy_type=policy_type,
        mode=mode,
    ...
    **kwargs,
    ...
    .. code-block:: yaml
        ...
        Ensure policy assignment exists:
            azurerm_resource.policy_assignment_present:
                - name: testassign
    ...
    policy_kwargs = kwargs.copy()
    policy_kwargs.update(connection_auth)
    policy = __salt__["azurerm_resource.policy_assignment_create"](
        name=name,
        scope=scope,
        definition_name=definition_name,
        type=assignment_type,
